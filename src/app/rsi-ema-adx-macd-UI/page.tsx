...
// Thay 7 const *_PERIOD cũ bằng 2 state: settings đang áp dụng + draft đang gõ
const [settings, setSettings] = useState<IndicatorSettings>(DEFAULT_SETTINGS);
const [draftSettings, setDraftSettings] = useState<IndicatorSettings>(DEFAULT_SETTINGS);
...
*/
function resetIndicatorStreams(s: IndicatorSettings) {
  rsiStreamRef.current = new RSIStream(s.rsiPeriod);
  macdStreamRef.current = new MACDStream(s.macdFast, s.macdSlow, s.macdSignal);
  ema20StreamRef.current = new EMAStream(s.emaFastPeriod);
  ema50StreamRef.current = new EMAStream(s.emaSlowPeriod);
  adxStreamRef.current = new ADXStream(s.adxPeriod);
}

//resetIndicatorStreams(settings)

async function applySettings() {
  const cleaned: IndicatorSettings = {
    rsiPeriod: Math.max(2, Math.round(draftSettings.rsiPeriod) || 14),
    emaFastPeriod: Math.max(2, Math.round(draftSettings.emaFastPeriod) || 20),
    emaSlowPeriod: Math.max(2, Math.round(draftSettings.emaSlowPeriod) || 50),
    macdFast: Math.max(2, Math.round(draftSettings.macdFast) || 12),
    macdSlow: Math.max(2, Math.round(draftSettings.macdSlow) || 26),
    macdSignal: Math.max(2, Math.round(draftSettings.macdSignal) || 9),
    adxPeriod: Math.max(2, Math.round(draftSettings.adxPeriod) || 14),
  };
  ...
  setSettings(cleaned);
  setDraftSettings(cleaned);

  if (!feedRef.current) return;

  await feedRef.current.reset();
  historyRef.current.clear();
  resetIndicatorStreams(cleaned);
  setTotalRead(0);
  setStatus("Đã áp dụng cài đặt mới — phát lại từ đầu");
  refresh();
}
...
>
  <label>
    ...
    <input
      ...
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, rsiPeriod: Number(e.target.value) }))
      }
      style={{ width: 55 }}
  ...
  <label>
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.emaFastPeriod}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, emaFastPeriod: Number(e.target.value) }))
      }
      style={{ width: 50 }}
      disabled={busy || autoPlay}
    />
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.emaSlowPeriod}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, emaSlowPeriod: Number(e.target.value) }))
      }
      style={{ width: 50 }}
      disabled={busy || autoPlay}
    />
  ...
  <label>
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.macdFast}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, macdFast: Number(e.target.value) }))
      }
      style={{ width: 45 }}
      disabled={busy || autoPlay}
    />
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.macdSlow}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, macdSlow: Number(e.target.value) }))
      }
      style={{ width: 45 }}
      disabled={busy || autoPlay}
    />
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.macdSignal}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, macdSignal: Number(e.target.value) }))
      }
      style={{ width: 45 }}
      disabled={busy || autoPlay}
    />
  ...
  <label>
    ...
    <input
      type="number"
      min={2}
      value={draftSettings.adxPeriod}
      onChange={(e) =>
        setDraftSettings((s) => ({ ...s, adxPeriod: Number(e.target.value) }))
      }
      style={{ width: 55 }}
      disabled={busy || autoPlay}
    />
  ...
  <button onClick={applySettings} disabled={busy || autoPlay}>
    Áp dụng & phát lại từ đầu
  </button>
</div>
